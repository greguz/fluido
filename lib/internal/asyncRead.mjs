...
export function createReadMethod (asyncRead) {
  ...
  return function _read (size) {
    ...
    const callback = once((err, data) => {
      ...
      const alive = !this._readableState.closed && !this._readableState.ended
      if (!alive) {
        return
      }

      if (err) {
        // This will set ._readableState.closed to true
        return this.destroy(err)
      }

      if (data !== undefined) {
        // If null, this will set ._readableState.ended to true
        this.push(data)
      }
