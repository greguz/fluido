...
export interface ReadableOptions {
   ...
   * on itself after ending. Default: false.
   ...
   * specified encoding. Default: null.
   * https://nodejs.org/api/stream.html#stream_new_stream_readable_options
   */
  encoding?: string;
  /**
   * The maximum number of bytes to store in the internal buffer before ceasing
   * to read from the underlying resource. Default: 16384 (16kb), or 16 for objectMode streams.
   * https://nodejs.org/api/stream.html#stream_new_stream_readable_options
   */
  highWaterMark?: number;
  /**
   * Whether this stream should behave as a stream of objects. Meaning that stream.read(n)
